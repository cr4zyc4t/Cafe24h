...
//        mTitleOffset = (int) (TITLE_OFFSET_DIPS * getResources().getDisplayMetrics().density); DEFAULT OFFSET
        mTitleOffset = Utils.getScreenWidth(context) / 2;
    ...
    /**
     * Set the {@link ViewPager.OnPageChangeListener}. When using {@link MySlidingTabLayout} you are
    ...
    private void scrollToTab(int tabIndex, int positionOffset) {
        ...
        if (selectedChild != null) {
            ...
            if (tabIndex > 0 || positionOffset > 0) {
                ...
                Pair<Float, Float> lines = getIndicatorCoordinates(tabIndex);
                targetScrollX += ((lines.second - lines.first) / 2);
            }
    ...
    private Pair<Float, Float> getIndicatorCoordinates(int currentPosition) {
        // default: line below current tab
        View currentTab = mTabStrip.getChildAt(currentPosition);
        float lineLeft = currentTab.getLeft();
        float lineRight = currentTab.getRight();

        // if there is an offset, start interpolating left and right coordinates between current and next tab
        if (currentPositionOffset > 0f && currentPosition < mTabStrip.getChildCount() - 1) {

            View nextTab = mTabStrip.getChildAt(currentPosition + 1);
